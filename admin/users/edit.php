...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $chk = $conn->query("SELECT id FROM admin_users WHERE email = '$emailEsc' AND id != $id");
        if ($chk && $chk->num_rows > 0) $errors[] = 'Email is already used by another user.';
    }
    ...
    if (empty($errors)) {
        // ── FIX 1: Correct bind_param type strings ────────────
        if (!empty($password)) {
            $hashed = password_hash($password, PASSWORD_BCRYPT);
            // Params: name(s), email(s), password(s), role(s), phone(s), avatar(s), status(i), two_fa(i), notes(s), id(i) = 10 params
            $stmt = $conn->prepare("UPDATE admin_users SET name=?, email=?, password=?, role=?, phone=?, avatar=?, status=?, two_fa_enabled=?, notes=?, updated_at=NOW() WHERE id=?");
            $stmt->bind_param("ssssssiisi", $name, $email, $hashed, $role, $phone, $avatar, $status, $two_fa, $notes, $id);
        } else {
            // Params: name(s), email(s), role(s), phone(s), avatar(s), status(i), two_fa(i), notes(s), id(i) = 9 params
            $stmt = $conn->prepare("UPDATE admin_users SET name=?, email=?, role=?, phone=?, avatar=?, status=?, two_fa_enabled=?, notes=?, updated_at=NOW() WHERE id=?");
            $stmt->bind_param("sssssiisi", $name, $email, $role, $phone, $avatar, $status, $two_fa, $notes, $id);
        }
        if ($stmt->execute()) {
            // ── FIX 2: Safe activity log insert ───────────────
            $ip     = $conn->real_escape_string($_SERVER['REMOTE_ADDR'] ?? '0.0.0.0');
            $act    = $conn->real_escape_string("Updated user: $name ($email) role: $role");
            $tblChk = $conn->query("SHOW TABLES LIKE 'admin_activity_log'");
...
// ── FIX 2: Safe activity log fetch ───────────────────────────
$logs   = [];
$tblChk = $conn->query("SHOW TABLES LIKE 'admin_activity_log'");
if ($tblChk && $tblChk->num_rows > 0) {
    $logRes = $conn->query("SELECT * FROM admin_activity_log WHERE user_id = $id ORDER BY created_at DESC LIMIT 5");
        ...
        <div class="content container-fluid pt-4 pb-5">
            ...
            <div class="row g-4">
                ...
                <div class="col-xl-8 col-lg-7">
                    ...
                    <div class="card border-0 shadow-sm rounded-4 mb-4" style="background: linear-gradient(135deg,#0d6efd12,#6f42c112);">
                        <div class="card-body p-4 d-flex align-items-center gap-4">
                            ...
                            <div class="avatar-upload-wrap flex-shrink-0" id="avatarWrap">
                                ...
                                <div class="avatar-placeholder shadow" id="avatarFallback" style="display:none;background:<?= $avatarColor ?>;">
                                    <?= $initials ?>
                                </div>
                                <?php else: ?>
                                <div class="avatar-placeholder shadow" id="avatarFallback" style="background:<?= $avatarColor ?>;">
                                    <?= $initials ?>
                                </div>
                                <?php endif; ?>
                                <div class="avatar-upload-btn shadow" onclick="document.getElementById('avatarInput').click()">
                            ...
                            <div class="flex-grow-1">
                                <div class="d-flex align-items-center gap-2 mb-1 flex-wrap">
                                    ...
                                    ?>
                                    <span class="badge <?= $rb ?> rounded-pill px-3 py-1 fw-semibold text-capitalize"><?= htmlspecialchars($user['role']) ?></span>
                                ...
                                <div class="row g-2">
                                    ...
                                    <div class="col-4">
                                        <div class="stat-mini">
                                            <div class="val"><?= $user['two_fa_enabled'] ? '<i class="fa fa-shield-alt text-success" style="font-size:1rem;"></i>' : '<i class="fa fa-times text-danger" style="font-size:1rem;"></i>' ?></div>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="input-group">
                                        ...
                                        <input type="tel" name="phone" class="form-control border-start-0 ps-0"
                                               value="<?= $p('phone') ?>" placeholder="+91 98765 43210">
                                    </div>
                                </div>
                                <div class="col-md-6">
                    ...
                    <div class="card border-0 shadow-sm rounded-4 mb-4">
                        <div class="card-header bg-white border-bottom py-3 px-4 d-flex align-items-center justify-content-between">
                            ...
                            <span class="badge bg-secondary-subtle text-secondary-emphasis border rounded-pill px-3 py-1 fw-medium small">Optional</span>
                        </div>
                        <div class="card-body p-4">
                ...
                <div class="col-xl-4 col-lg-5">
                    ...
                    <div class="card border-0 shadow-sm rounded-4 mb-4">
                        ...
                        <div class="card-body p-4">
                            ...
                            <div class="p-3 bg-light rounded-3 border mb-3 small">
                                <div class="text-muted fw-bold text-uppercase mb-2" style="font-size:.68rem;letter-spacing:.4px;">Last Login</div>
                            ...
                            <div class="p-3 bg-light rounded-3 border small">
                                <div class="text-muted fw-bold text-uppercase mb-2" style="font-size:.68rem;letter-spacing:.4px;">Password Reset Token</div>
                                <?php
                    ...
                    <div class="card border-0 shadow-sm rounded-4 mb-4">
                        ...
                        <div class="card-body p-4">
                            ...
                            <div class="log-item">
                                <div class="small fw-semibold text-dark"><?= htmlspecialchars(ucwords(str_replace('_', ' ', $log['action']))) ?></div>
                                <div class="text-muted" style="font-size:.72rem;">
                                    <?= date('M d, Y h:i A', strtotime($log['created_at'])) ?>
                                    <?php if (!empty($log['ip'])): ?> &mdash; <?= htmlspecialchars($log['ip']) ?><?php endif; ?>
                                </div>
                            </div>
                            <?php endforeach; ?>
                            <?php endif; ?>
